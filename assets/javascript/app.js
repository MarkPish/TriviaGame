...
var questions = [{
    ...
    //slide9a.jpg
 {
    ...
    //slide10a.jpg
 {
    question: "Which mountain range is located in northern Italy?",
    choices: ["Urals", "Himalayas", "Dolomites", "Apennines", "Pyrenees"],
    correctAnswer: 2 },
    //slide11a.jpg
 {
...
$('#next').on('click', function(){
    ...
    if (isNaN(selections[questionNum])) {
...
// set up time frame and temp file
    ...
    // Need to create question and chosen answer divs
    ...
    //radio buttons...can choose only one option...research use     
     ...
     function choose() {
         selections[questionNum] = +$('input[name="answer"]:checked').val();
     }
     ...
     function displayNext() {
         quiz.fadeOut(function() {
             ...
             if (questionNum < questions.length) {
                 var nextQuestion = makeQuiz(questionNum);
                 quiz.append(nextQuestion).show();
                 if (!(isNaN(selections[questionNum]))) {
                     $('input[value='+selections[questionNum]+']').prop('checked', true);
                 }
     ...
     //compute score and return a paragraph element to be displayed
